...
<div style="margin: 15px 0; padding: 15px; background-color: #e9f7ef; border-radius: 6px; border-left: 4px solid #27ae60;">
<h5 style="color: #2c3e50; margin-bottom: 8px;">📈 Efficiency Metrics Explained</h5>

<p><strong>VRAM Efficiency</strong>: Measures how effectively your job uses the allocated GPU memory. It's calculated as the ratio of <em>maximum memory used</em> to <em>total memory allocated</em>. Higher values (closer to 1.0) indicate more efficient resource utilization.</p>

<p><strong>VRAM Hours</strong>: This metric represents the product of job duration (in hours) and allocated VRAM (in GiB), giving you a measure of total GPU memory consumption over time. Lower VRAM hours for the same workload indicate more efficient resource usage.</p>

<p>Optimizing these metrics can help reduce resource contention, lower costs, and improve overall cluster efficiency.</p>
</div>
...
### What is Allocated VRAM Efficiency?

Allocated VRAM efficiency measures how effectively your job uses the GPU memory you requested. It is calculated as:

$Efficiency = \frac{\text{VRAM Used (GiB)}}{\text{Allocated VRAM (GiB)}}$

A value close to 1.0 means you are using most of the memory you requested; a lower value means you are over-allocating memory relative to your actual needs.
...
if 'alloc_vram_efficiency' in user_jobs.columns:
    ...
    user_efficiency = user_jobs['alloc_vram_efficiency'] * 100
    ...
    user_avg_efficiency = user_efficiency.mean() / 100  # Convert back to 0-1 scale
    other_users_efficiencies = other_users_efficiency / 100  # Convert to 0-1 scale

    # Calculate what percentile the user is in
    percentile_rank = (other_users_efficiencies < user_avg_efficiency).mean() * 100

    print(f"- **Your percentile rank**: {percentile_rank:.1f}th percentile")

    # Add interpretation of percentile
    if percentile_rank >= 90:
        percentile_msg = "🏆 **Top 10%** - Excellent performance!"
    elif percentile_rank >= 75:
        percentile_msg = "🥇 **Top 25%** - Very good performance"
    elif percentile_rank >= 50:
        percentile_msg = "📊 **Above median** - Better than half of users"
    elif percentile_rank >= 25:
        percentile_msg = "📉 **Below median** - Room for improvement"
    else:
        percentile_msg = "🎯 **Bottom 25%** - Significant optimization needed"

    print(f"- **Ranking**: {percentile_msg}")

    import seaborn as sns
    plt.figure(figsize=(6,4))
    sns.boxplot(data=other_users_efficiencies.clip(upper=1), color='#4ECDC4')
    plt.scatter(0, user_avg_efficiency, color='orange', zorder=5, label='Your Efficiency')  # Highlight user's efficiency in orange
    plt.ylabel('Efficiency')
    plt.title('Efficiency Percentile Distribution')
    plt.legend()
    plt.show()

    # Efficiency distribution
    very_low_threshold = EfficiencyCategoryEnum.VERY_LOW_THRESHOLD.value * 100  # 10%
    low_threshold = EfficiencyCategoryEnum.LOW_THRESHOLD.value * 100  # 30%
    good_threshold = EfficiencyCategoryEnum.GOOD_THRESHOLD.value * 100  # 70%

    very_low_eff = sum(user_efficiency < very_low_threshold)
    low_eff = sum((user_efficiency >= very_low_threshold) & (user_efficiency < low_threshold))
    medium_eff = sum((user_efficiency >= low_threshold) & (user_efficiency < good_threshold))
    high_eff = sum(user_efficiency >= good_threshold)

    print()
    print(f"**Your Efficiency Distribution:**")
    print()
    print(f"- **Very low efficiency jobs** (< {very_low_threshold:.0f}%): {very_low_eff} ({(very_low_eff/len(user_efficiency)*100):.1f}%)")
    print()
    print(f"- **Low efficiency jobs** ({very_low_threshold:.0f}%-{low_threshold:.0f}%): {low_eff} ({(low_eff/len(user_efficiency)*100):.1f}%)")
    print()
    print(f"- **Medium efficiency jobs** ({low_threshold:.0f}%-{good_threshold:.0f}%): {medium_eff} ({(medium_eff/len(user_efficiency)*100):.1f}%)")
    print()
    print(f"- **High efficiency jobs** (≥ {good_threshold:.0f}%): {high_eff} ({(high_eff/len(user_efficiency)*100):.1f}%)")
```
...
if len(comparison_stats) > 0:
    ...
    efficiency_metrics = comparison_stats[comparison_stats['Category'].str.contains('efficiency|Efficiency', case=False, na=False)]
    ...
    if len(efficiency_metrics) > 0:
        ...
        ax.set_ylim(0, 100 * 1.1)  # 110% to allow space for labels
...
```{python}
#| echo: false
#| fig-cap: "Time Usage Metrics Comparison"
#| fig-width: 12
#| fig-height: 6

if len(comparison_stats) > 0:
    # Focus only on time-related metrics, excluding VRAM efficiency since it's already shown above
    time_metrics = comparison_stats[
        comparison_stats['Category'].str.contains(
            'time.*usage|time.*efficiency|ratio|utilization|average.*memory.*per', 
            case=False, na=False
        )
    ]
    
    # Exclude VRAM efficiency metrics since they're already displayed in the previous section
    time_metrics = time_metrics[
        ~time_metrics['Category'].str.contains('VRAM.*efficiency|efficiency.*VRAM|alloc.*vram.*efficiency', case=False, na=False)
    ]
    
    # Remove pure memory amounts (GiB) which cause scale issues
    time_metrics = time_metrics[
        ~time_metrics['Category'].str.contains('Total.*Memory|GPU.*Memory.*GiB|GB', case=False, na=False)
    ]
    
    if len(time_metrics) > 0:
        fig, ax = plt.subplots(figsize=(12, 6))
        
        categories = time_metrics['Category']
        your_values = time_metrics['Your_Value']
        avg_values = time_metrics['Average_Value']
        ...
        ax.set_xlabel('Time Usage Metrics', fontweight='bold')
        ax.set_ylabel('Percentage (%)', fontweight='bold')
        ax.set_title('Time Usage Performance Comparison', fontweight='bold', fontsize=14)
        ax.set_xticks(x)
        ...
        print("No time usage metrics available for comparison.")
else:
    print("No comparison data available.")
```
...
<details>
<summary><h2 style="display: inline-block; margin: 0;">Additional Analysis</h2></summary>
...
if len(time_series_data) > 0:
    ...
    if 'JobCount' in time_series_data.columns:
        fig, ax = plt.subplots(figsize=(12, 6))
        
        # Get the appropriate time column
        x_col = 'TimeGroup_Datetime' if 'TimeGroup_Datetime' in time_series_data.columns else 'TimeGroup_Str'
        
        # Convert to datetime if needed
        if x_col == 'TimeGroup_Datetime' and time_series_data[x_col].dtype == 'object':
            time_series_data[x_col] = pd.to_datetime(time_series_data[x_col])
        
        # Create a clean line plot with markers
        ax.plot(time_series_data[x_col], time_series_data['JobCount'], 
        ...
        ax.fill_between(time_series_data[x_col], time_series_data['JobCount'], 
                       alpha=0.3, color='#90EE90')
        
        # Add horizontal line for average
        avg_jobs = time_series_data['JobCount'].mean()
        ...
        if x_col == 'TimeGroup_Datetime':
            ...
            n_ticks = min(8, len(time_series_data))
            tick_indices = np.linspace(0, len(time_series_data)-1, n_ticks, dtype=int)
            ax.set_xticks([time_series_data[x_col].iloc[i] for i in tick_indices])
            plt.xticks(rotation=45)
...
<details>
<summary><h4>📊 ROC Efficiency Analysis</h4></summary>
