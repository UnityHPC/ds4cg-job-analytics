...
user_jobs_df = pd.DataFrame(user_jobs)  # Use the already loaded user_jobs instead of data['user_jobs']
        ...
        if missing_cols:
            ...
            if len(other_users) >= 5:
                ...
                if 'alloc_vram_efficiency' in user_jobs_df.columns:
                    ...
                    # Show efficiency quartiles for context
                    q25 = np.percentile(other_users_efficiencies * 100, 25)
                    q50 = np.percentile(other_users_efficiencies * 100, 50)
                    q75 = np.percentile(other_users_efficiencies * 100, 75)
                    
                    print()
                    # print(f"**System Efficiency Quartiles:**")
                    # print(f"- **25th percentile**: {q25:.1f}% (bottom quartile)")
                    # print(f"- **50th percentile (median)**: {q50:.1f}% (middle)")
                    # print(f"- **75th percentile**: {q75:.1f}% (top quartile)")
                    # print(f"- **Your efficiency**: {user_efficiency.mean():.1f}%")
                    
                    # Efficiency distribution using enum constants
...
if len(comparison_stats_df) > 0:
    ...
    time_metrics = comparison_stats_df[
