...
if len(user_jobs) > 0 and 'StartTime' in user_jobs.columns:
    try:
        ...
        if not time_series_data.empty and len(time_series_data) > 2:
...
<details>
<summary><h4>📊 Efficiency Threshold Curve</h4></summary>
...
if not roc_available:
    print("Efficiency Threshold analysis not available.")
else:
...
if len(a100_jobs) > 0 and len(user_jobs) > 0:
    try:
        ...
        # Drop rows with NaN or Inf in columns you will plot
        cols_to_check = ['alloc_vram_efficiency', 'used_vram_gib', 'vram_hours', 'JobID', 'JobPercentage']
        efficiency_metrics = efficiency_metrics.replace([np.inf, -np.inf], np.nan).dropna(subset=cols_to_check)
...
if len(a100_jobs) > 0:
    ...
    try:
        ...
        <h5 style="color: #495057; margin-bottom: 15px;">💾 A100 Resource Usage</h5>
        """
        ...
        if total_a100_hours > 0:
            ...
            if len(other_jobs) > 0:
                ...
                html_content += f"""
                <h6 style="color: #495057; margin: 15px 0 10px 0;">📊 Resource Distribution</h6>
                <ul>
        ...
        html_content += """
        <h6 style="color: #495057; margin: 15px 0 10px 0;">💡 A100-Specific Recommendations</h6>
        <ul>
        """
        
        if avg_a100_efficiency < 0.3:
            html_content += """
            <li>🔧 <strong>Optimize Memory Usage</strong>: Your A100 efficiency is low. Consider using batch size optimization or mixed precision training.</li>
            <li>🔄 <strong>Consider Alternatives</strong>: For jobs with low memory requirements, regular GPUs might be more cost-effective.</li>
            """
        
        if avg_a100_memory < 20:  # Less than 20GB average usage
            html_content += """
            <li>📊 <strong>Memory Underutilization</strong>: You're using less than half of A100's memory capacity. Consider larger models or batch sizes.</li>
            """
        
        if len(a100_jobs) < 5:
            html_content += """
            <li>📈 <strong>Limited A100 Experience</strong>: Consider experimenting with larger workloads that can fully utilize A100 capabilities.</li>
            """
        else:
            html_content += """
            <li>✅ <strong>Regular A100 User</strong>: You're making good use of A100 resources. Continue optimizing for better efficiency.</li>
            """
