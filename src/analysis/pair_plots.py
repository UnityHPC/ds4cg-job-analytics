...
def get_db_dataframe(db_path=None, table_name="Jobs"):
    ...
    dataframe = db.con.execute(f"SELECT * FROM {table_name}").df()
    return dataframe


def plot_selected_pairs(jobs_dataframe, pairs=None, hue=None, sample=1000, plot_kws=None):
    """
    Plot selected pairs of columns from a DataFrame using seaborn scatterplots.

    Args:
        jobs_dataframe (pd.DataFrame): The DataFrame to plot from.
    ...
    if sample and len(jobs_dataframe) > sample:
        jobs_dataframe = jobs_dataframe.sample(sample, random_state=42)
    if plot_kws is None:
    ...
    for i, (x, y) in enumerate(pairs):
        ax = axes[i]
        sns.scatterplot(data=jobs_dataframe, x=x, y=y, hue=hue, ax=ax, **plot_kws)
