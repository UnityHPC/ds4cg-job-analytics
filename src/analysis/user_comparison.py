...
class UserComparison:
    ...
    def __init__(self, db_connection: DatabaseConnection) -> None:
        ...
        try:
            self._all_jobs_data = load_and_preprocess_jobs(
                db_path=self.db.db_url,
    ...
    def get_user_metrics(self, user_id: str) -> pd.DataFrame:
        ...
        try:
            ...
            user_jobs = jobs[jobs["User"] == user_id]
    ...
    def get_other_users_metrics(self, exclude_user: str) -> pd.DataFrame:
        ...
        if self._cached_all_users_metrics is None:
            try:
                ...
                all_jobs = load_and_preprocess_jobs(
                    db_path=self.db.db_url,
                    include_cpu_only_jobs=False,
                    include_failed_cancelled_jobs=False,
                    min_elapsed_seconds=0,
    ...
    def get_user_comparison_statistics(
        ...
        for metric_name, _ in metrics:
            if metric_name == "alloc_vram_efficiency" or metric_name == "requested_vram_efficiency":
                user_metrics[metric_name] = user_jobs[metric_name].clip(upper=1).mean() * 100  # Convert to percentage
