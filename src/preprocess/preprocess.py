...
class Preprocess:
    ...
    The preprocessing class method is preprocess_data().
    """
    ...
    def _validate_gpu_type(
        cls, gpu_type_value: NAType | np.ndarray | list | dict, include_cpu_only_jobs: bool
    ) -> list | dict | NAType:
        """
    ...
    def _safe_apply_function(
        cls, func: Callable, *args: object, job_id: int | None = None, idx: int | None = None
    ) -> int | NAType:
        """
    ...
    @classmethod
    def _fill_missing(cls, res: pd.DataFrame, include_cpu_only_jobs: bool) -> None:
        """
        Intended for internal use inside class only. Fill missing values in the DataFrame with default values.
    ...
    def _write_preprocessing_error_logs(cls, preprocessing_error_logs: list[dict]) -> None:
        ...
        print(
            ...
            f"Reporting them to a summary file "
            f"{PREPROCESSING_ERRORS_LOG_FILE if not cls.anonymize else PREPROCESSING_ERRORS_LOG_FILE.name}."
        )
    ...
    @classmethod
    def anonymize_str_column(cls, column: pd.Series, prefix: str) -> pd.Series:
