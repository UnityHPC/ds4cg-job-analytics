...
def test_pre_process_data_fill_missing_Interactive_mock2(load_small_data_2):
    data = preprocess_data(data=load_small_data_2, min_elapsed=100, include_CPU_only_job=True, include_failed_cancelled_jobs=True)
...
def test_pre_process_data_filter_min_esplapes_mock2(load_small_data_2):
    data = preprocess_data(data=load_small_data_2, min_elapsed=700, include_CPU_only_job=True, include_failed_cancelled_jobs=True)
    assert len(data) == 8