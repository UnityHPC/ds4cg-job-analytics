...
def check_quarto() -> tuple[bool, str]:
    ...
    try:
        ...
        result = subprocess.run(["quarto", "--version"], capture_output=True, text=True, check=False)
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            result = subprocess.run(
                ["quarto", "render", test_file, "--to", "html"], capture_output=True, text=True, check=False
            )
            if result.returncode == 0:
...
def main() -> None:
    ...
    for module in REQUIRED_MODULES:
        ...
        if result is True:
            ...
            print(f"  ✗ {module} load failed. Please install it with: pip install {module}")
    ...
    if (
        all_modules_ok
        and quarto_result[0]
        and check_file_exists(template_path)
