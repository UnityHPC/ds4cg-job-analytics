...
from src.analysis.pi_group_analysis import PIGroupAnalysis
...
def generate_pi_group_report(
    ...
    summary_stats = pd.DataFrame({
        ...
        "Value": [
            ...
            f"{efficiency:.1%}",
            efficiency_category,
            f"{metrics_row['avg_alloc_vram_efficiency_score']:.2f}" if pd.notna(metrics_row['avg_alloc_vram_efficiency_score']) and metrics_row['avg_alloc_vram_efficiency_score'] != -np.inf else "Very Low",
            analysis_period
        ]
    })
...
def generate_pi_group_recommendations(pi_account: str, metrics: pd.Series, pi_users: pd.DataFrame, worst_users: pd.DataFrame) -> list[str]:
    """
    ...
    if efficiency < EfficiencyCategoryEnum.VERY_LOW_THRESHOLD.value:
        ...
        recommendations.append(
            "**Efficiency Improvement Priority**: Focus on optimizing GPU memory usage patterns across your research group. "
            "Consider implementing group-wide best practices for memory allocation."
        )

    # Group size-based recommendations
    if user_count >= 10:
        recommendations.append(
            "**Large Group Management**: With a large research group, consider establishing group-wide GPU usage guidelines "
            "and regular efficiency reviews to maintain optimal resource utilization."
        )
    elif user_count <= 2:
        recommendations.append(
            "**Small Group Optimization**: As a small research group, focus on intensive optimization of existing workflows "
            "to maximize the impact of your GPU resource usage."
        )
    ...
    if len(worst_users) > 0:
        ...
        if low_eff_count > 0:
            recommendations.append(
                ...
                f"Consider providing targeted GPU optimization guidance to: {', '.join(worst_users.head(3)['User'].tolist())}."
            )
...
def generate_reports_for_specific_pi_groups(
    ...
    for i, pi_account in enumerate(pi_list, 1):
        ...
        if pi_metrics.iloc[0]['job_count'] < min_jobs:
            print(f"      ❌ {pi_account}: Not enough jobs ({pi_metrics.iloc[0]['job_count']} found, minimum required: {min_jobs})")
            continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--template", type=str, default="./reports/pi_group_report_template.qmd", help="Path to the Quarto template file"
    )
    parser.add_argument(
        "--format",
        type=str,
